...
Wallet.withdraw = function (user, address, amount, next) {
  // Check balance of user
  Wallet.getBalance(user, function (balance) {
    console.log(balance)
    console.log(amount)
    if (balance >= amount + coin.txFee) { // Enough balance
      Wallet.rpc.cmd('settxfee', coin.txFee, function (err, res) { // Set tx fee
        if (err) next()
        else {
          Wallet.rpc.cmd('sendfrom', user, address, amount, 4, function (err, res) {
            if (err) next()
            else next(res)
          })  
        }
